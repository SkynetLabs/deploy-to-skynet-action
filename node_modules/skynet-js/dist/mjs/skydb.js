...
import { DEFAULT_GET_ENTRY_OPTIONS, DEFAULT_SET_ENTRY_OPTIONS, validatePublicKey, } from "./registry";
import { checkCachedDataLink, DEFAULT_GET_JSON_OPTIONS, DEFAULT_SET_ENTRY_DATA_OPTIONS, DEFAULT_SET_JSON_OPTIONS, DELETION_ENTRY_DATA, validateEntryData, } from "./skydb_v2";
import { formatSkylink } from "./skylink/format";
import { BASE64_ENCODED_SKYLINK_SIZE, decodeSkylink, EMPTY_SKYLINK, RAW_SKYLINK_SIZE } from "./skylink/sia";
import { DEFAULT_UPLOAD_OPTIONS } from "./upload";
...
import { hexToUint8Array, stringToUint8ArrayUtf8, toHexString, trimUriPrefix, uint8ArrayToStringUtf8, } from "./utils/string";
 ...
 */
export async function getEntryData(publicKey, dataKey, customOptions) {
    validatePublicKey("publicKey", publicKey, "parameter");
    validateString("dataKey", dataKey, "parameter");
 ...
 */
export async function setEntryData(privateKey, dataKey, data, customOptions) {
    validateHexString("privateKey", privateKey, "parameter");
    validateString("dataKey", dataKey, "parameter");
 ...
 */
export async function deleteEntryData(privateKey, dataKey, customOptions) {
    // Validation is done below in `db.setEntryData`.
 ...
 * @throws - Will throw if the revision is already the maximum value.
 */
export async function getNextRegistryEntry(client, publicKey, dataKey, data, customOptions) {
    // Not publicly available, don't validate input.
    const opts = {
        ...DEFAULT_GET_ENTRY_OPTIONS,
        ...client.customOptions,
 ...
 */
export function getNextRevisionFromEntry(entry) {
    let revision;
    if (entry === null) {
        revision = BigInt(0);
    }
