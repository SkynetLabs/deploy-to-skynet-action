...
export async function deleteJSON(privateKey, dataKey, customOptions) {
    ...
    const entry = await getNextRegistryEntry(this, toHexString(publicKeyArray), dataKey, new Uint8Array(RAW_SKYLINK_SIZE), getEntryOpts);
    // Update the registry.
    const setEntryOpts = extractOptions(opts, DEFAULT_SET_ENTRY_OPTIONS);
    await this.registry.setEntry(privateKey, entry, setEntryOpts);
}
...
export async function setDataLink(privateKey, dataKey, dataLink, customOptions) {
    ...
    const { publicKey: publicKeyArray } = sign.keyPair.fromSecretKey(hexToUint8Array(privateKey));
    const getEntryOpts = extractOptions(opts, DEFAULT_GET_ENTRY_OPTIONS);
    const entry = await getNextRegistryEntry(this, toHexString(publicKeyArray), dataKey, decodeSkylink(dataLink), getEntryOpts);
    // Update the registry.
    const setEntryOpts = extractOptions(opts, DEFAULT_SET_ENTRY_OPTIONS);
    await this.registry.setEntry(privateKey, entry, setEntryOpts);
}
