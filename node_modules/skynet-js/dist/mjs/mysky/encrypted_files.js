...
import { throwValidationError, validateBoolean, validateHexString, validateNumber, validateObject, validateString, validateUint8Array, validateUint8ArrayLen, } from "../utils/validation";
/**
 ...
 */
export function decryptJSONFile(data, key) {
    ...
    if (metadata.version !== ENCRYPTED_JSON_RESPONSE_VERSION) {
        throw new Error(`Received unrecognized JSON response version '${metadata.version}' in metadata, expected '${ENCRYPTED_JSON_RESPONSE_VERSION}'`);
    }
 ...
 */
export function encryptJSONFile(json, metadata, key) {
 ...
 */
export function deriveEncryptedFileKeyEntropy(pathSeed) {
 ...
 */
export function deriveEncryptedFileTweak(pathSeed) {
    // Validate the path seed and get bytes.
    const pathSeedBytes = validateAndGetFilePathSeedBytes(pathSeed);
 ...
 */
export function deriveEncryptedFileSeed(pathSeed, subPath, isDirectory) {
 ...
 * @throws - Will throw if the input sub path is not a valid path.
 */
export function deriveEncryptedPathSeed(pathSeed, subPath, isDirectory) {
    ...
    if (pathSeed.length !== ENCRYPTION_PATH_SEED_DIRECTORY_LENGTH) {
        throwValidationError("pathSeed", pathSeed, "parameter", `a directory path seed of length '${ENCRYPTION_PATH_SEED_DIRECTORY_LENGTH}'`);
    }
 ...
 */
export function padFileSize(initialSize) {
    const kib = 1 << 10;
 ...
 */
export function encodeEncryptedFileMetadata(metadata) {
...
function validateAndGetFilePathSeedBytes(pathSeed) {
    ...
    if (pathSeed.length !== ENCRYPTION_PATH_SEED_FILE_LENGTH) {
        throwValidationError("pathSeed", pathSeed, "parameter", `a valid file path seed of length '${ENCRYPTION_PATH_SEED_FILE_LENGTH}'`);
    }
